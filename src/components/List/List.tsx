...
import { ItemProps, ListProps, TEvent } from '../../ts/types';

const AUTOSCROLL_ACTIVE_OFFSET = 200;
const AUTOSCROLL_SPEED_RATIO = 10;

class List<Value = string> extends React.Component<ListProps<Value>> {
  ...
  constructor(props: ListProps<Value>) {
    super(props);
  ...
  onMouseOrTouchStart = (e: MouseEvent & TouchEvent) => {
    ...
    // @ts-ignore
    if (index === -1 || (this.props.values[index] && this.props.values[index].disabled)) {
