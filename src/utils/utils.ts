...
export function getTranslateOffset(element: Element) {
  ...
  return (
    Math.max(parseInt(style['margin-top' as any], 10), parseInt(style['margin-bottom' as any], 10)) + element.getBoundingClientRect().height
  );
}

export function transformItem(element: Element, offsetY: number | null = 0, offsetX: number | null = 0) {
  if (!element) return;
...
export function setItemTransition(element: Element, duration: number, timing?: string) {
  if (element) {
    (element as HTMLElement).style['transition' as any] = `transform ${duration}ms${timing ? ` ${timing}` : ''}`;
  }
}
...
export function checkIfInteractive(target: Element, rootElement: Element) {
  const DISABLED_ELEMENTS = ['input', 'textarea', 'select', 'option', 'optgroup', 'video', 'audio', 'button', 'a'];
